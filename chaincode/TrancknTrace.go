...
func (t *TnT) createAssembly(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	//Checking if the Assembly already exists
		...
		/* Retreive Block Chain HashID ------------------------------------------Starts */

		/*size, err1 := ledger.GetBlockchainSize()
		if err1 != nil {
		return nil, err1
		}
		if size > 0 {
		currentBlock, err1 := ledger.GetBlockByNumber(size - 1)
		if err1 != nil {
			return nil, err1
		}
		assem.AssemblyBlockHashId = currentBlock.GetHash(); //Add the current block chain hash to the assembly
		}*/
		
		/* Retreive Block Chain HashID ------------------------------------------Ends */
